...
const CANDY_MACHINE_V2_PROGRAM = new web3Js.PublicKey(
  'cndy3Z4yapfJBmL3ShUp5exZKqR3z33thTzeNMm2gRZ',
);

async function getCandyMachineCreator(
  candyMachine: web3Js.PublicKey,
): Promise<[web3Js.PublicKey, number]> {
...
export async function getCandyMachineMints(
  candyMachineId: string,
  env: string = 'mainnet-beta',
...
export async function getAccountsByCreatorAddress(
  creatorAddress: web3Js.PublicKey,
  connection: web3Js.Connection,
): Promise<(string | Metadata)[][]> {
  const metadataAccounts = await getProgramAccounts(connection, TOKEN_METADATA_PROGRAM.toBase58(), {
    filters: [
      {
        memcmp: {
          offset:
...
export async function getAddressesByCreatorAddress(
  ...
  try {
    const spinner = ora({
      text: 'Calling rpc to get mints, please wait',
